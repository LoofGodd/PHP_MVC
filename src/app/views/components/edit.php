...
<form class="bg-secondary bg-opacity-10 mx-2 py-3 rounded-5 row align-items-end  text-center column-gap-3" action="/update" method="POST" enctype="multipart/form-data">
    <div class='col-2 row'>
        <input type='text' hidden name=<?= ID ?> value="<?= $showPost[ID] ?>" />
        ...
        <?php if (Image::read($showPost[IMAGE])): ?>
    ...
    <div class="col-3">
        <input type="text" name=<?= HEADING ?> class="form-control" id="headingId" value="<?php echo $showPost[HEADING] ?>" />
    </div>
    <div class="col-3">
        <input type="text" name=<?= DESCRIPTION ?> class="form-control" id="descriptionId" value="<?php echo $showPost[DESCRIPTION] ?>">
    </div>
    <div class='col-3'>
